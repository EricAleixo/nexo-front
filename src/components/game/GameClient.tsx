...
function AnsweredPlayersList({ answeredList, options, newEntryId }: {
      ...
      {answeredList.map((entry) => {
        ...
        return (
          <div
            ...
            className={[
              "relative flex items-center gap-1.5 rounded-full border px-2 py-1 transition-all duration-300",
              color.label.replace("bg-", "border-").replace("/30", "/40"),
              "bg-zinc-900/80",
              isNew ? "scale-110 ring-2 ring-white/20" : "scale-100",
            ].join(" ")}
            style={{ animation: isNew ? "popIn 0.35s cubic-bezier(0.34,1.56,0.64,1) forwards" : undefined }}
          >
            ...
            <span className="text-[11px] font-semibold text-white/80 max-w-[60px] truncate">{entry.playerName}</span>
            <span className={`text-[10px] font-black rounded-md px-1 ${color.label}`}>{OPTION_LABELS[optIdx] ?? "?"}</span>
          </div>
        );
      })}
    </div>
...
function HostOptionViewer({ options, voteCounts, totalPlayers, phase, correctOptionId, answeredList, newEntryId }: {
  options: Option[];
      ...
      {options.map((opt, i) => {
        ...
        return (
          <div
            key={opt.id}
            className={[
              "relative flex flex-col gap-2.5 rounded-2xl border px-5 py-4 bg-gradient-to-br transition-all duration-300",
              isCorrect ? "border-emerald-500/60 from-emerald-500/20 to-emerald-600/10 text-emerald-200 shadow-[0_0_24px_rgba(52,211,153,.2)]" : color.base,
              hasNewInThisOption && phase === "answering" ? "shadow-[0_0_16px_rgba(255,255,255,.06)]" : "",
            ].join(" ")}
          >
            <div className="flex items-center gap-4">
              <span className={`flex size-8 shrink-0 items-center justify-center rounded-xl text-xs font-black ${isCorrect ? "bg-emerald-500/30 text-emerald-300" : color.label}`}>
                {OPTION_LABELS[i]}
              </span>
              <span className="flex-1 text-sm font-semibold leading-snug">{opt.title}</span>
              {isCorrect && <svg className="size-5 shrink-0 text-emerald-400" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>}
            </div>

            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 rounded-full bg-white/10 overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-700 ${isCorrect ? "bg-emerald-400" : color.bar}`} style={{ width: `${pct}%` }} />
              </div>
            ...
            {optionVoters.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-0.5">
                {optionVoters.map((entry) => {
                  const isNew = entry.playerId === newEntryId;
                  return (
                    <div key={entry.playerId} title={entry.playerName} className="relative flex items-center justify-center" style={{ animation: isNew ? "popIn 0.35s cubic-bezier(0.34,1.56,0.64,1) forwards" : undefined }}>
                      <span className={`size-6 rounded-full ${getAvatarColor(entry.playerName)} flex items-center justify-center text-[10px] font-black text-white ring-2 ${isNew ? "ring-white/40" : "ring-zinc-900"} transition-all duration-200`}>
                        {entry.playerName.charAt(0).toUpperCase()}
                      </span>
                      {isNew && <span className={`absolute inset-0 rounded-full ${color.pulse} opacity-60`} style={{ animation: "ping 0.6s ease-out forwards" }} />}
                    </div>
                  );
                })}
              </div>
...
export function GameClient({ code, questions, timePerQuestion, players: initialPlayers }: Props) {
  ...
  // ── Score do jogador atual (reativo — atualiza quando players muda via room:state) ──
  const myCurrentScore = players.find((p) => p.id === currentPlayer?.id)?.score ?? 0;
  ...
  const startTimer = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) { stopTimer(); setPhase("result"); return 0; }
        return prev - 1;
      });
    }, 1000);
  ...
  useEffect(() => {
    ...
    function onRoomState(state: { players: Player[]; currentPlayer: CurrentPlayer }) {
      setPlayers(state.players);
      setCurrentPlayer(state.currentPlayer);
    }

    function onAnswerResult(data: { correctOptionId: string; players: Player[] }) {
      stopTimer();
    ...
    function onNextQuestion(data: { index: number }) {
      if (data.index < questions.length) { setCurrentIndex(data.index); }
      else { router.push(`/room/${code}/finish`); }
    }

    function onTimeExpired(data: { correctOptionId: string; players: Player[] }) {
      setCorrectOptionId(data.correctOptionId);
      setPlayers(data.players);
      setPhase("result");
    }
    ...
    function onVoteUpdate(data: {
      ...
      latestVote: { playerId: string; playerName: string; optionId: string; isNew: boolean };
    }) {
    ...
    function onGameFinished() { router.push(`/room/${code}/finish`); }
    ...
    socket.on("time_expired_result", onTimeExpired);
    ...
    return () => {
      ...
      socket.off("time_expired_result", onTimeExpired);
  ...
  function handleSelectOption(optionId: string) {
    if (isHost || phase !== "answering" || selectedOptionId === optionId) return;
    setSelectedOptionId(optionId);
    socketRef.current?.emit("submit_answer", { questionId: question.id, optionId, timeLeft });
  }

  function handleNextQuestion() {
    socketRef.current?.emit("next_question", { index: currentIndex + 1 });
  }
  ...
  return (
    <>
      <style>{`
        @keyframes popIn { 0% { transform: scale(0.4); opacity: 0; } 70% { transform: scale(1.15); opacity: 1; } 100% { transform: scale(1); opacity: 1; } }
        @keyframes ping { 0% { transform: scale(1); opacity: 0.6; } 100% { transform: scale(2.2); opacity: 0; } }
        @keyframes bumpScale { 0% { transform: scale(1); } 40% { transform: scale(1.35); } 100% { transform: scale(1); } }
        .bump { animation: bumpScale 0.35s cubic-bezier(0.34,1.56,0.64,1) forwards; }
      `}</style>

      <div className="fixed inset-0 bg-[#080810] -z-10" />
      <div aria-hidden className="fixed inset-0 -z-10 pointer-events-none" style={{ backgroundImage: "linear-gradient(rgba(99,102,241,.04) 1px, transparent 1px), linear-gradient(90deg, rgba(99,102,241,.04) 1px, transparent 1px)", backgroundSize: "56px 56px" }} />
      <div aria-hidden className="fixed top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] -z-10 pointer-events-none opacity-10" style={{ background: "radial-gradient(ellipse, #6366f1 0%, #a855f7 50%, transparent 80%)", filter: "blur(80px)" }} />
          ...
          <div className="mx-auto flex max-w-3xl items-center justify-between px-4 sm:px-6 py-3 gap-4">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="w-32 sm:w-48 h-1.5 rounded-full bg-zinc-800 overflow-hidden">
                <div className="h-full rounded-full bg-indigo-500 transition-all duration-500" style={{ width: `${((currentIndex + 1) / questions.length) * 100}%` }} />
              </div>
            </div>

            <span className="text-sm font-black tracking-widest text-zinc-600" style={{ fontFamily: "var(--font-syne, monospace)" }}>{code}</span>

            {isHost ? (
              <div className="flex items-center gap-2 rounded-full border border-purple-500/25 bg-purple-500/10 px-3 py-1.5 shrink-0">
                <svg className="size-3.5 text-purple-400" fill="currentColor" viewBox="0 0 24 24"><path d="M5 16L3 5l5.5 5L12 4l3.5 6L21 5l-2 11H5zm2 3h10v1H7v-1z" /></svg>
              ...
              <div className="flex items-center gap-2 rounded-full border border-indigo-500/25 bg-indigo-500/10 px-3 py-1.5 shrink-0">
                <svg className="size-3.5 text-indigo-400" fill="currentColor" viewBox="0 0 24 24"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.17.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.64-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
                {/* ── Score reativo do jogador atual ── */}
                <span className="text-xs font-black text-indigo-300 tabular-nums">{myCurrentScore} pts</span>
              </div>
            )}
          </div>
        </header>
              ...
              <div className="rounded-2xl border border-zinc-800/60 bg-zinc-900/60 backdrop-blur-sm px-6 sm:px-8 py-6 flex flex-col gap-4">
                <div className="flex items-start gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    {/* ── Título da pergunta + pontuação que ela vale ── */}
                    <div className="flex items-center gap-2 mb-2">
                      <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-500">
                        Pergunta {currentIndex + 1}
                      </p>
                      {question.score > 0 && (
                        <span className="inline-flex items-center gap-1 rounded-md bg-amber-500/15 border border-amber-500/25 px-1.5 py-0.5 text-[10px] font-black text-amber-400">
                          +{question.score} pts
                        </span>
                      )}
                    </div>
                    <h1 className="text-base sm:text-xl font-bold text-white leading-snug">{question.title}</h1>
                  </div>
                  {isHost && phase === "answering" && (
                    <div className="shrink-0 flex flex-col items-center gap-0.5">
                      <span key={totalVotesReceived} className={`text-2xl font-black text-white tabular-nums ${countBump ? "bump" : ""}`}>{totalVotesReceived}</span>
                      <span className="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider">/{guestPlayers.length}</span>
                    </div>
                  )}
                </div>
                {isHost && phase === "answering" && answeredList.length > 0 && (
                  <div className="border-t border-zinc-800/60 pt-3">
                    <AnsweredPlayersList answeredList={answeredList} options={question.options} newEntryId={newEntryId} />
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {question.options.map((opt, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className={["flex size-8 shrink-0 items-center justify-center rounded-xl text-xs font-black transition-colors", state === "correct" ? "bg-emerald-500/30 text-emerald-300" : state === "wrong" ? "bg-red-500/20 text-red-400" : state === "selected" ? "bg-indigo-500/30 text-indigo-300" : state === "dim" ? "bg-zinc-800 text-zinc-600" : color.label].join(" ")}>
                        {OPTION_LABELS[i]}
                      </span>
                      <span className="flex-1 text-sm font-semibold leading-snug">{opt.title}</span>
            ...
            {!isHost && selectedOptionId && phase === "answering" && (
              <div className="flex items-center justify-center gap-3 rounded-2xl border border-indigo-500/20 bg-indigo-500/8 px-6 py-4">
                <div className="flex gap-1">
                  {[0, 1, 2].map((i) => <span key={i} className="size-1.5 rounded-full bg-indigo-500/60 animate-bounce" style={{ animationDelay: `${i * 150}ms` }} />)}
                </div>
          ...
          <main className="flex-1 w-full animate-in fade-in duration-500">
            <div className="mx-auto max-w-5xl px-4 sm:px-6 py-8 flex flex-col lg:flex-row gap-6 h-full">
              ...
              <div className="flex flex-col gap-4 lg:w-[44%] shrink-0">
                <div className="rounded-2xl border border-zinc-800/60 bg-zinc-900/60 px-5 py-4">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-500">Pergunta {currentIndex + 1}</p>
                    {question.score > 0 && (
                      <span className="inline-flex items-center gap-1 rounded-md bg-amber-500/15 border border-amber-500/25 px-1.5 py-0.5 text-[10px] font-black text-amber-400">
                        +{question.score} pts
                      </span>
                  ...
                  <p className="text-base font-bold text-white leading-snug">{question.title}</p>
                </div>
                ...
                <div className="flex flex-col gap-2.5">
                  {question.options.map((opt, i) => {
                    ...
                    return (
                      <div
                        key={opt.id}
                        className={["flex items-center gap-4 rounded-2xl border px-5 py-4 bg-linear-to-br transition-all duration-500",
                          isCorrect ? "border-emerald-500/60 from-emerald-500/20 to-emerald-600/10 text-emerald-200 shadow-[0_0_28px_rgba(52,211,153,.18)]"
                            : wasSelected && !isCorrect ? "border-red-500/30 from-red-500/10 to-red-600/5 text-red-300 opacity-60"
                              : `${color.base} opacity-50`,
                        ].join(" ")}
                      >
                        <span className={`flex size-9 shrink-0 items-center justify-center rounded-xl text-sm font-black ${isCorrect ? "bg-emerald-500/30 text-emerald-300" : wasSelected ? "bg-red-500/20 text-red-400" : color.label}`}>{OPTION_LABELS[i]}</span>
                        <span className="flex-1 text-sm font-semibold leading-snug">{opt.title}</span>
                        {isCorrect && <svg className="size-5 shrink-0 text-emerald-400" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>}
                        {wasSelected && !isCorrect && <svg className="size-5 shrink-0 text-red-400" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>}
                      </div>
                    );
                  })}
                </div>

                {/* ── Feedback de resultado para o jogador ── */}
                {!isHost && (
                  <div className={`flex items-center gap-3 rounded-2xl border px-5 py-3.5 ${!selectedOptionId ? "border-zinc-700/40 bg-zinc-900/50" : selectedOptionId === correctOptionId ? "border-emerald-500/30 bg-emerald-500/10" : "border-red-500/25 bg-red-500/8"}`}>
                    <div className={`flex size-9 items-center justify-center rounded-xl shrink-0 ${!selectedOptionId ? "bg-zinc-800" : selectedOptionId === correctOptionId ? "bg-emerald-500/20" : "bg-red-500/15"}`}>
                      {!selectedOptionId
                    ...
                    <div>
                      <p className={`text-sm font-bold ${!selectedOptionId ? "text-zinc-300" : selectedOptionId === correctOptionId ? "text-emerald-300" : "text-red-300"}`}>
                      ...
                      <p className="text-xs text-zinc-500 mt-0.5">
                        {!selectedOptionId
                          ...
                          : selectedOptionId === correctOptionId
                            ? `+${question.score} pts adicionados ao seu placar` // ← usa question.score
                            : "Não foi dessa vez"}
                      </p>
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-4 flex-1 min-h-0">
                <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-500 text-center">Placar</p>
                ...
                <div className="flex flex-col gap-2 overflow-y-auto flex-1">
                  {[...players]
                    ...
                    .map((p, i) => (
                      <div
                        key={p.id}
                        className={`flex items-center gap-3 rounded-xl px-4 py-3.5 border transition-all ${p.id === currentPlayer?.id ? "border-indigo-500/40 bg-indigo-500/10" : "border-zinc-800/60 bg-zinc-900/60"}`}
                      >
                        <span className={`text-sm font-black w-6 shrink-0 tabular-nums ${i === 0 ? "text-amber-400" : i === 1 ? "text-zinc-400" : i === 2 ? "text-amber-700" : "text-zinc-600"}`}>#{i + 1}</span>
                        <span className={`size-8 rounded-full ${getAvatarColor(p.name)} flex items-center justify-center text-sm font-black text-white shrink-0`}>{p.name.charAt(0).toUpperCase()}</span>
                        ...
                        {/* ── Score atualizado via room:state/answer_result ── */}
                ...
                {isHost ? (
                  <button
                    ...
                    className="group relative flex items-center justify-center gap-3 overflow-hidden rounded-2xl bg-linear-to-r from-indigo-500 to-purple-600 px-8 py-4 text-sm font-black text-white w-full shadow-[0_0_32px_rgba(99,102,241,.35)] hover:shadow-[0_0_48px_rgba(99,102,241,.55)] hover:from-indigo-400 hover:to-purple-500 active:scale-[.97] transition-all duration-300"
                  >
                    {isLastQuestion
                      ? <><Trophy className="size-4" />Ver resultado final</>
                      : <>Próxima pergunta<svg className="size-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /></svg></>
                    }
                    <span aria-hidden className="pointer-events-none absolute inset-0 -translate-x-full bg-linear-to-r from-transparent via-white/15 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
                  </button>
                ) : (
                  <div className="flex items-center justify-center gap-3 rounded-2xl border border-zinc-800/60 bg-zinc-900/40 px-6 py-4">
                    <div className="flex gap-1">
                      {[0, 1, 2].map((i) => <span key={i} className="size-1.5 rounded-full bg-zinc-600 animate-bounce" style={{ animationDelay: `${i * 150}ms` }} />)}
                    </div>
